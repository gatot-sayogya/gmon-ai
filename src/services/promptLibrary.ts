...
const STORAGE_KEY = 'gmon_ai_user_prompts';
const PIN_LIMIT = 20;
...
export const promptLibraryService = {
    ...
    getPinnedPreConfiguredPrompts: (): string[] => {
        try {
            const stored = localStorage.getItem('gmon_ai_pinned_preconfigured');
            return stored ? JSON.parse(stored) : [];
        } catch (e) {
    ...
    togglePreConfiguredPin: (content: string): boolean => {
        ...
        localStorage.setItem('gmon_ai_pinned_preconfigured', JSON.stringify(newPinned));
