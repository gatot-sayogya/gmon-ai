...
test('ChatInterface should render and allow sending messages', async ({ page, mockLLMHealth, waitForPortal }) => {
    ...
    await expect(page.getByText('Hello Gmon AI')).toBeVisible();
});
...
test('ChatHistory should allow pinning and unpinning conversations', async ({ page, waitForPortal }) => {
    ...
    await page.evaluate(() => {
        ...
        localStorage.setItem('gmon_ai_chat_history', JSON.stringify([session]));
    });

    await page.goto('/a/gmon-ai/history');
...
test('ChatHistory should delete conversation after confirmation', async ({ page, waitForPortal }) => {
    ...
    await page.evaluate((title) => {
        ...
        const existing = JSON.parse(localStorage.getItem('gmon_ai_chat_history') || '[]');
        existing.push(session);
        localStorage.setItem('gmon_ai_chat_history', JSON.stringify(existing));
    }, uniqueTitle);
...
test('ChatHistory should filter conversations with search', async ({ page, waitForPortal }) => {
    ...
    await page.evaluate(() => {
        ...
        const existing = JSON.parse(localStorage.getItem('gmon_ai_chat_history') || '[]');
        localStorage.setItem('gmon_ai_chat_history', JSON.stringify([...existing, ...sessions]));
    });
...
test('ChatHistory should display persisted thinking duration when loading conversation', async ({ page, waitForPortal }) => {
    ...
    await page.evaluate(() => {
        ...
        localStorage.setItem('gmon_ai_chat_history', JSON.stringify([mockSession]));
    });
