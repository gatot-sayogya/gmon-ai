...
// App is the Gmon AI plugin instance.
...
func NewApp(_ context.Context, settings backend.AppInstanceSettings) (instancemgmt.Instance, error) {
	...
	app.tracer = tracerProvider.Tracer("gmon-ai")

	// Initialize metrics using our custom global store
	meterProvider := GetMeterProvider()
	meter := meterProvider.Meter("gmon-ai")

	var err error
	app.chatRequestsTotal, err = meter.Int64Counter(
		"gmon_ai.chat.requests.total",
	...
	app.llmFirstTokenLatency, err = meter.Float64Histogram(
		"gmon_ai.llm.first_token_latency",
